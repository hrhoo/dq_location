...
@st.cache_data
def load_data():
    df_by_prov = pd.read_csv("projects/python/crawler/DQ/DQ_location_gby_p.csv")
    df_location = pd.read_csv("projects/python/crawler/DQ/DQ_location_with_no_dup.csv")
    df_by_city = pd.read_csv("projects/python/crawler/DQ/DQ_location_gby_c.csv")
    return df_by_prov, df_location, df_by_city

# Load the data
@st.cache_data
def load_geojson():
    with open("projects/python/crawler/DQ/Streamlit/canada_provinces.geo.json", encoding="utf-8") as json_data:
...
def main():
    ...
    with st.container():
        st.image("projects/python/crawler/DQ/Streamlit/DQ_logo.png", width=100)
    st.title(APP_NAME)
